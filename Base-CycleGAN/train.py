...
parser.add_argument('--n_epochs', type = int, default = 120, help = 'number of epochs training')
parser.add_argument('--batchSize', type = int, default = 3, help = 'size of the batch sizes')
parser.add_argument('--dataroot', type = str, default = './pizza/A', help='root directory of the dataset')
parser.add_argument('--lr',type = float, default = 0.0002, help = 'initial learning rate')
parser.add_argument('--decay_epoch', type = int, default = 100, help = 'epoch to start linearly decaying the learning rate to 0')
parser.add_argument('--size', type = int, default = 256, help = 'size of the data crop (squared assumed)')
parser.add_argument('--input_nc', type = int, default = 3, help = 'number of channels of the input data')
parser.add_argument('--output_nc', type = int, default = 3, help = 'number of channels of the output data')
...
testdataloader = DataLoader(ImageDataset(opt.dataroot, transforms_ = transforms_, unaligned = True, mode = 'test' ), batch_size  = opt.batchSize, shuffle = True, num_workers = opt.n_cpu)
print(")))))))))))))))))")
#### Loss Plot ##############################
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        print({'loss_G': loss_G.item(), 'loss_G_identity': loss_G_identity.item(), 'loss_G_GAN': loss_G_GAN.item(),
                    'loss_G_cycle': loss_G_cycle.item(), 'loss_D': loss_D.item()})
                    # , 
                    # images={'real_A': real_A, 'real_B': real_B, 'fake_A': fake_A, 'fake_B': fake_B})
        loss_dict[curr] = {}
        loss_dict[curr]['loss_G'] = loss_G
        loss_dict[curr]['loss_G_identity'] = (loss_identity_A + loss_identity_B)
        loss_dict[curr]['loss_G_GAN'] = (loss_GAN_A2B + loss_GAN_B2A)
        loss_dict[curr]['loss_G_cycle'] = (loss_cycle_ABA + loss_cycle_BAB)
        loss_dict[curr]['loss_D'] = (loss_D_A + loss_D_B)
        curr += 1
